defmodule MishkaGervaz.Table.Web.Events.RelationFilterHandler do
  ...
  @doc false
  def get_filter(params, state) do
  ...
  @doc false
  def get_search_term(params, state, filter_atom) do
  ...
  @doc false
  def get_toggle_params(params, state) do
  ...
  @doc false
  def get_current_filter_opts(state, filter_atom) do
    Map.get(state.relation_filter_state, filter_atom, %{})
  end

  @doc false
  def get_current_selected_list(state, filter_atom) do
  ...
  @doc false
  def to_filter_map(filter) when is_struct(filter), do: Map.from_struct(filter)
  def to_filter_map(filter) when is_map(filter), do: filter

  @doc false
  def resolve_selected_options(filter_map, state, filter_atom) do
    current_opts = get_current_filter_opts(state, filter_atom)
  ...
  @doc false
  def toggle_value(current_list, value, true = _is_selected) do
    Enum.reject(current_list, &(&1 == value))
  end

  def toggle_value(current_list, value, false = _is_selected) do
    current_list ++ [value]
  end

  @doc false
  def toggle_selected_option(state, filter_atom, value, label, is_selected) do
    current_opts = get_current_filter_opts(state, filter_atom)
  ...
  @doc false
  def build_toggle_state(state, filter_atom, new_list, new_selected_options) do
    current_opts = get_current_filter_opts(state, filter_atom)
  ...
  @doc false
  def update_relation_state(socket, state, filter_atom, result, opts \\ []) do
    current_opts = get_current_filter_opts(state, filter_atom)
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      def handle(action, params, state, socket) do
        MishkaGervaz.Table.Web.Events.RelationFilterHandler.do_handle(
          action,
          params,
          state,
          socket
