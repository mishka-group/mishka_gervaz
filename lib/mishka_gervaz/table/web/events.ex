defmodule MishkaGervaz.Table.Web.Events do
  ...
  def relation_filter_handler(state) do
  ...
  def sanitize(state, value), do: sanitization_handler(state).sanitize(value)

  @doc false
  @spec sanitize_column(State.t(), String.t()) :: atom()
  def sanitize_column(state, column), do: sanitization_handler(state).sanitize_column(column)

  @doc false
  @spec sanitize_page(State.t(), String.t() | integer()) :: pos_integer()
  def sanitize_page(state, page), do: sanitization_handler(state).sanitize_page(page)
  ...
  def destroy_record(state, record, action) do
  ...
  def update_record(state, record, action) do
  ...
  def execute_bulk_action(bulk_action, selected_ids, state, socket) do
  ...
  def apply_hook_result(state, hook_name, args, default_socket) do
  ...
  def run_action_hook(state, phase, action_name, args) do
  ...
  def apply_action_hook_socket(state, phase, action_name, args, default_socket) do
  ...
  def adapt_hook_args(state, hook_key, args, socket) do
  ...
  def halted_before_action?(state, phase, action_name, args) do
  ...
  @spec do_handle(String.t(), map(), State.t(), Phoenix.LiveView.Socket.t()) ::
          {:noreply, Phoenix.LiveView.Socket.t()}
  def do_handle("sort", %{"column" => column}, state, socket) do
  ...
  def do_handle("filter", params, state, socket) do
    filter_values =
  ...
  def do_handle("clear_filters", _params, state, socket) do
    socket =
      %{State.update(state, filter_values: %{}) | relation_filter_state: %{}}
      |> then(&DataLoader.load_async(socket, &1, page: 1, reset: true))
  ...
  def do_handle("archive_filter", params, state, socket) do
  ...
  def do_handle("load_more", _params, state, socket) do
    socket = DataLoader.load_more(socket, state)
    {:noreply, socket}
  end

  def do_handle("prev_page", _params, state, socket) do
    if state.page > 1 do
  ...
  def do_handle("next_page", _params, state, socket) do
  ...
  def do_handle("go_to_page", %{"page" => page}, state, socket) do
  ...
  def do_handle("change_page_size", %{"size" => size}, state, socket) do
  ...
  def do_handle("destroy", params, state, socket) do
  ...
  def do_handle("show_modal", %{"id" => id}, state, socket) do
    id = sanitize(state, id)
    send(self(), {:show_modal, id})
    {:noreply, socket}
  end

  def do_handle("edit_modal", %{"id" => id}, state, socket) do
    id = sanitize(state, id)
    send(self(), {:edit_modal, id})
    {:noreply, socket}
  end

  def do_handle("show_versions", %{"id" => id}, state, socket) do
    id = sanitize(state, id)
    send(self(), {:show_versions, id})
    {:noreply, socket}
  end

  def do_handle("row_action", params, state, socket) do
  ...
  def do_handle("custom_event", params, state, socket) do
    event_name = sanitize(state, params["event"])
  ...
  def do_handle("expand_row", %{"id" => id}, state, socket) do
    id = sanitize(state, id)
  ...
  def do_handle("close_expanded", _params, state, socket) do
    old_expanded_id = state.expanded_id
    state = State.update(state, expanded_id: nil, expanded_data: nil)
  ...
  def do_handle("toggle_select", %{"id" => id}, state, socket) do
    id = sanitize(state, id)
  ...
  def do_handle("toggle_select_all", _params, state, socket) do
  ...
  def do_handle("switch_template", %{"template" => template_name}, state, socket) do
    template_atom =
  ...
  def do_handle("bulk_action", %{"action" => action_name}, state, socket) do
  ...
  @doc false
  def do_edit(state, %{"id" => id}, socket) do
  ...
  def find_row_action_by_event(state, event_name) do
  ...
  def do_update(
        state,
        %{"id" => _id, "_action_name" => action_name} = params,
        action_spec,
        socket
      ) do
  ...
  def do_update(state, %{"id" => _id} = params, action_spec, socket) do
  ...
  def update_action_name({_master, tenant}), do: tenant
  def update_action_name(action) when is_atom(action), do: action
  ...
  @spec safe_stream_reinsert(Phoenix.LiveView.Socket.t(), State.t(), map() | nil) ::
          Phoenix.LiveView.Socket.t()
  def safe_stream_reinsert(socket, _state, nil), do: socket
  ...
  @spec hide_row(Phoenix.LiveView.Socket.t(), State.t(), String.t()) ::
          Phoenix.LiveView.Socket.t()
  def hide_row(socket, state, id) do
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      def handle(event, params, socket) do
        ...
        MishkaGervaz.Table.Web.Events.do_handle(event, params, state, socket)
      end
