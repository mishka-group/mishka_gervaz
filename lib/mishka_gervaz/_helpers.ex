defmodule MishkaGervaz.Helpers do
  ...
  @doc """
  Returns the resource's Ash calculations as a `%{name => calc_struct}` map.
  """
  ...
  @doc """
  Returns the resource's Ash relationships as a list.
  """
  ...
  @doc """
  Picks a "display field" name from the resource's attributes.

  Prefers `:name`, then `:title`, then `:label`; falls back to `:id` when
  none are present. Used by relation/select rendering for human labels.
  """
  ...
  @doc """
  Resolves an `:options` field on a filter/field map.

  When `options` is a 0-arity function, calls it; otherwise returns the
  filter unchanged.
  """
  ...
  @doc """
  Extracts the underlying records list from an Ash read result.

  Handles `%Ash.Page.Offset{}` / `%Ash.Page.Keyset{}` (anything with `:results`),
  raw lists, and falls back to `[]` for unknown shapes.
  """
  ...
  @doc """
  Like `map_put_if_set/3` but for keyword lists. Drops nil values.
  """
  ...
  @doc """
  Resolves the tenant attribute value for a record on a multitenant resource.

  Returns the value of the multitenancy attribute on the record, or `nil`
  when the resource is not multitenant.
  """
  ...
  @doc """
  Returns the multitenancy attribute name for the resource a record belongs to.

  Returns `nil` for non-struct values or resources without multitenancy.
  """
  ...
  @doc """
  Default record-visibility check based on tenant matching.

  Master users (no `tenant_field` value) see all; otherwise the record's
  tenant must match the user's, or be unset.
  """
  ...
  @doc """
  Returns the next step name in a list of step structs after `current`.

  Returns `nil` when `current` is unknown or already last.
  """
  ...
  @doc """
  Returns the previous step name in a list of step structs before `current`.

  Returns `nil` when `current` is unknown or already first.
  """
  ...
  @doc """
  Returns true if `step_name` exists in the form state's steps list.
  """
  ...
  @doc """
  Resolves an upload name to its namespaced LiveView form upload name.

  When the form has a Static `id`, returns the namespaced name produced by
  `MishkaGervaz.Form.Web.UploadHelpers.namespaced_upload_name/2`. Otherwise
  returns the bare upload key as-is.
  """
